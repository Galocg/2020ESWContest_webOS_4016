...
# 1-3) 재탐색 신호가 들어오지 않고 오랫동안 request를 줘야하는 상황이면 어떻게 할 것인가? (쓸데없이 트래픽을 많이 주게 됨) -> re_detect = true 가 올 때 까지는 리퀘스트 주기를 천천히함
...
def decoder(result) :
    ...
    if result['re_detect'] == 'true' :
        result['re_detect'] = True
    else 
...
def giveserver(num_of_dog, detect_recommend) :

    #get형식으로 통신

    uri = url +'/home/trace?' 
    data = dict()
    data.update({"num_of_dog" : num_of_dog})
    data.update({"detect_recommend" : detect_recommend})
    print(data)
